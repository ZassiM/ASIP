...
extern "C++"  signed int  mul_called (signed int,signed int) property(functional loop_free);

// 32b
inline    signed int  operator *  (  signed int a,   signed int b) property(functional loop_free) {
  return mul_called(a,b);
}

inline  unsigned int  operator *  (unsigned int a, unsigned int b) property(functional loop_free) {
  return mul_called((  signed int)a,(  signed int)b);
}
...
// emulation function in libtrv32p3
extern "C++"    signed int  div_called (  signed int,  signed int,chess_output   signed int&) property(functional loop_free);
extern "C++"  unsigned int  div_called (unsigned int,unsigned int,chess_output unsigned int&) property(functional loop_free);

// additional indirection - to enable common subexpression identification
inline  signed int  div_called_ (signed int a, signed int b, chess_output signed int& r) property(functional loop_free) {
  return div_called(a,b,r);
}
inline  unsigned int  div_called_ (unsigned int a, unsigned int b, chess_output unsigned int& r) property(functional loop_free) {
  return div_called(a,b,r);
}

inline  signed int  operator / (signed int a, signed int b) property(llvm_emulated) {
  signed int r;
  return div_called_(a,b,r);
}

inline  unsigned int  operator / (unsigned int a, unsigned int b) property(llvm_emulated) {
  unsigned int r;
  return div_called_(a,b,r);
}
