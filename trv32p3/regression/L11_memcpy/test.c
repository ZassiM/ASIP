...
void gcc_torture_memcpy_2()
{
    ...
    for (int off1 = 0; off1 < MAX_OFFSET; ++off1) {
        for (int off2 = 0; off2 < MAX_OFFSET; ++off2) {
            for (int len = 1; len < 2 * MAX_OFFSET; ++len) {
    ...
    if (step < 1) step = 1;
    for (int off1 = 0; off1 < MAX_OFFSET; off1 += step) {
        for (int off2 = 0; off2 < MAX_OFFSET; off2 += step) {
            for (int len = 6 * MAX_OFFSET; len < 8 * MAX_OFFSET; ++len) {
                inner_loop(off1, off2, len);
            }
        }
    }
