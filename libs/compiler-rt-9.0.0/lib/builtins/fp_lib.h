...
static __inline int rep_clz(rep_t a) { return overloaded_clz(a); }
...
#if defined __LP64__
  return overloaded_clz(a);
#else
  if (a & REP_C(0xffffffff00000000))
    return overloaded_clz((uint32_t)(a >> 32));
  else
    return 32 + overloaded_clz((uint32_t)(a & REP_C(0xffffffff)));
#endif
}
